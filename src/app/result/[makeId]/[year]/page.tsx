	...
	return (
		<>
			<div className="flex flex-row-reverse justify-between items-start flex-wrap">
				...
				<h1 className="flex-grow font-bold text-5xl">{lookup[makeId]?.value?.makeName || 'Unknown make'}</h1>
			</div>

			<h2 className="font-bold text-xl">Year: {year || 'unknown'}</h2>

			<div>
				<label className="font-bold">Has {models.length} {plural('model', models)}:</label>
